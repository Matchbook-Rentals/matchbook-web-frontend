...
import { HostTermsAgreementForm } from "./host-terms-agreement-form";

export default async function HostTermsPage({ searchParams }: { searchParams: { redirect_url?: string } }) {
  console.log(`[HOST TERMS PAGE] Rendering with searchParams:`, searchParams);
  
  const { userId } = auth();
  console.log(`[HOST TERMS PAGE] User ID: ${userId}`);
  
  if (!userId) {
    console.log(`[HOST TERMS PAGE] No user ID, redirecting to sign-in`);
    return redirect("/sign-in");
  }
  
  console.log(`[HOST TERMS PAGE] Showing host terms page to user`);

  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <h1 className="text-3xl font-bold mb-6">Host Terms and Conditions</h1>
      ...
      <HostTermsAgreementForm redirectUrl={searchParams.redirect_url} />
    </div>
  );
}