...
interface GuestSearchFavoriteGridProps {
...
const GuestSearchFavoriteGrid: React.FC<GuestSearchFavoriteGridProps> = ({
  listings,
  ...
  // Create snapshot for guest cards to show proper like/dislike states
  const guestSnapshot = useMemo(() => ({
  ...
  }), [state.lookup.favIds, state.lookup.dislikedIds, optimisticLike, optimisticDislike, optimisticRemoveLike, optimisticRemoveDislike]);
  ...
  const getListingStatus = useCallback((listing: ListingAndImages) => {
    // Guests can't apply, so no "Applied" status
  ...
  const getCallToAction = useCallback((listing: ListingAndImages, status: ListingStatus) => {
    ...
    // Default fallback - should not happen since we pass cardActions from the tab
    return {
      label: 'Apply Now',
      action: () => actions.showAuthPrompt('apply', listing.id),
      className: 'bg-blueBrand text-white'
    };
  }, [withCallToAction, cardActions, actions]);
      ...
      {listings.length === 0 ? (
        ...
        <>
          ...
          >
            <div
              ref={gridRef}
              className="grid grid-cols-1 justify-items-start min-[800px]:grid-cols-2 min-[1200px]:grid-cols-3 min-[1600px]:grid-cols-4 gap-8 pb-12"
            >
...
export default GuestSearchFavoriteGrid;