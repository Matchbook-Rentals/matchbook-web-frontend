...
interface GuestFilterOptionsDialogProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  className?: string;
}

const GuestFilterOptionsDialog: React.FC<GuestFilterOptionsDialogProps> = ({
  isOpen,
  onOpenChange,
  className,
}) => {
  const { state: { filters: contextFilters, showListings, listings, session }, actions: { updateFilters } } = useGuestTripContext();
  ...
  // Calculate filtered listings count for live preview based on localFilters
  const filteredListingsCount = useMemo(() => {
    if (!listings || !session) return 0;

    // Create mock trip for price calculations
    const mockTrip = {
  ...
  const basicsOptions = [
    ...
    },
    {
      value: 'wifi',
      label: 'WiFi',
      icon: <Wifi className="h-[65px] w-[65px]" />
    },
    {
  ...
  const handleSave = () => {
    console.log('🔧 Guest Dialog handleSave called with localFilters:', localFilters);
    updateFilters(localFilters);
    // Clear map viewport filter to show all filtered listings, not just those in current viewport
...
export default GuestFilterOptionsDialog;