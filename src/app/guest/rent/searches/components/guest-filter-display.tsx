...
interface GuestFilterDisplayProps {
  className?: string;
  onOpenFilter?: () => void;
}

export const GuestFilterDisplay: React.FC<GuestFilterDisplayProps> = ({ className = "", onOpenFilter }) => {
  const { state: { filters, listings, showListings }, actions: { updateFilters } } = useGuestTripContext();
  ...
  // Calculate results for guests (no liked listings)
  const showListingsCount = showListings?.length || 0;
  const totalResults = showListingsCount;
