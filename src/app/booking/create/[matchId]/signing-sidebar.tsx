...
import type { FieldFormType } from '@/components/pdf-editor-v2/types';
import type { Recipient } from '@/components/pdf-editor-v2/RecipientManager';
...
export function SigningSidebar({
  ...
  // Find current signer by email (more robust than using numeric index)
  const currentSigner = currentRenterEmail
    ? recipients.find(r => r.email === currentRenterEmail)
    : recipients[1]; // Fallback to index 1 (first renter) for backward compatibility

  if (!currentSigner) {
    return null;
  }

  // Get the current signer's index from the recipients array
  const currentSignerIndex = recipients.findIndex(r => r.email === currentSigner.email);
  ...
  const completedFields = signerFields.filter(f => signedFields[f.formId] === 'signed');
            ...
            {signerFields.length === 0 ? (
              ...
              <div className="space-y-2">
                {signerFields.map(field => {
                  ...
                  const isCompleted = signedFields[field.formId] === 'signed';
