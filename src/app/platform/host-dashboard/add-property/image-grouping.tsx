...
const ImageGrouping: React.FC<ImageGroupingProps> = ({
  listingImages,
  onDragStart,
  ...
  const handleDragOver = useCallback(
    (e, img: ListingImage) => {
      ...
      const half =
        offsetX < (target as HTMLElement).offsetWidth / 2 ? "left" : "right";
  ...
  const handleDragLeaveImage = useCallback(() => {
    const lastImage =
      filteredAndSortedImages[filteredAndSortedImages.length - 1];
  ...
  return (
    <div onDragLeave={handleDragLeave} className="my-5">
      <div className="flex rounded-lg  text-xl border-2 border-gray-300 p-1 items-center justify-between cursor-pointer">
        ...
        <button
          onClick={toggleOpen}
          className="hover:bg-slate-300 px-3 transition transition-duration-400 font-bold text-2xl"
        >
          <motion.div
            ...
            <div key={img.id} className="relative w-full items-center flex">
              ...
              >
                <Image
                  ...
                  layout="fill"
                  objectFit="cover"
                  className={`transition transition-duration-400 ${dropPosition?.img?.id === img.id && dropPosition?.half === "left" ? "border-l-4 border-blue-500" : ""} ${dropPosition?.img?.id === img.id && dropPosition?.half === "right" ? "border-r-4 border-blue-500" : ""}`}
                />
                {dropPosition?.img?.id === img?.id && (
                  <div
                    className={`drop-indicator ${dropPosition?.half}`}
                    style={{
                      borderColor: "purple",
                      borderWidth: "2px",
                      [dropPosition?.half]: 0,
