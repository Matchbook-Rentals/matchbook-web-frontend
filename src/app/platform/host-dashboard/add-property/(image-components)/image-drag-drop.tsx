...
const ImageDragDrop: React.FC<ImageDragDropProps> = ({
  ...
  const removeImgFromOldCategory = (draggedImage: ListingImage) => {
    let removedDraggedArray = listingImages.filter(
      (img) => img.id !== draggedImage.id,
    );
    let oldCategoryArray = removedDraggedArray.filter(
      (img) => img.category === draggedImage.category,
    );
    let restOfArray = listingImages.filter(
      (img) => img.category !== draggedImage.category,
    );
    let sortedOldCategoryArray = oldCategoryArray.sort(
      (a, b) => a.rank - b.rank,
    );
  ...
  const insertToNewCategory = (
    ...
    let arrayWithSpaceAtRank = newCategoryArray.map(
      (img: ListingImage, idx: number) => {
  ...
  const handleDrop = (newCategory: string) => {
    ...
    if (over.img) {
      ...
      if (over.img.id === dragging.id && newCategory !== dragging?.category) {
        insertionRank =
          listingImages.filter((img) => img.category === newCategory).length +
          1;
      }
      ...
      insertionRank =
        listingImages.filter((img) => img.category === newCategory).length + 1;
    }

    setOver({ img: "", activeHalf: "" });
    let removedFromOldCategory = removeImgFromOldCategory(dragging);
    let insertedAtNewCategory = insertToNewCategory(
      removedFromOldCategory,
      newCategory,
      insertionRank,
    );
      ...
      {groupingCategories.map((category, idx) => (
        <ImageGrouping
          ...
          handleChangeCategory={(newCategory: string) =>
            handleChangeCategory(idx, newCategory)
          }
        />
      ))}
      <Button
        ...
        onClick={() =>
          setGroupingCategories([
            ...groupingCategories,
            `Room ${groupingCategories.length + 1}`,
          ])
        }
      >
        Add Room
