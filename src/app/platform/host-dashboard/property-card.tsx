...
const PropertyCard = ({ property }: PropertyCardProps) => {
  return (
    <Link
      ...
      className="max-w-sm flex flex-col justify-between rounded overflow-hidden shadow-lg bg-white"
    >
      <img
        className="w-full"
      ...
      />
      <div
        id="property-card-content"
        className="px-4 py-4 flex flex-col justify-evenly "
      >
        <div className="font-bold text-xl mb-2">{property.title}</div>
        <div className="flex flex-row items-end justify-between ">
          <p className="text-gray-700 text-base font-semibold">
            {property.locationString?.length &&
            property.locationString.length > LOCATION_STRING_MAX_LENGTH
          ...
          <span
            ...
            className="bg-primaryBrand px-2 py-1 text-sm font-semibold h-fit text-green-800 whitespace-nowrap"
          >
