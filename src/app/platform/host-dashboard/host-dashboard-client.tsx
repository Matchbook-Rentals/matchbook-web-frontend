...
const HostDashboardClient: React.FC<HostDashboardClientProps> = ({ properties }) => {
  ...
  const [filteredProperties, setFilteredProperties] = React.useState<Listing & { listingImages: ListingImage[] }[]>(properties);


  React.useEffect(() => {
    if (statusFilter) {
      setFilteredProperties(properties.filter(property => property.status === statusFilter));
    } else {
  ...
  return (
    <>
      <HostDashboardHeader properties={properties} statusFilter={statusFilter} setStatusFilter={setStatusFilter} />
