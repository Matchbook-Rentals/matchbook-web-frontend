...
const RoomsCounter: React.FC<RoomsCounterProps> = ({ goToNext, goToPrev, setUserPreferences }) => {
  ...
  return (
    <>
      <h2 className=' text-center text-2xl my-10 font-semibold'>What kind of place are you looking for?</h2>
      <div className='card border border-black flex flex-col w-1/2 mx-auto mt-5 rounded-2xl'>
        <Counter hasBorder label={'Bathrooms'} count={bathroomCount} onDecrement={() => setBathroomCount(prev => prev - 1)} onIncrement={() => setBathroomCount(prev => prev + 1)} />
        <Counter hasBorder={false} label={'Bedrooms'} count={bedroomCount} onDecrement={() => setBedroomCount(prev => prev - 1)} onIncrement={() => setBedroomCount(prev => prev + 1)} />
      </div>
