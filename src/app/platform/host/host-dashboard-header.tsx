...
const HostDashboardHeader = ({ properties, statusFilter, setStatusFilter }: HostDashboardHeaderProps) => {
  return (
        ...
        <div className='flex flex-wrap gap-3 justify-center sm:justify-start'>
          <Tabs>
            <div className="flex space-x-4">
              <Button className="text-2xl font-semibold" variant="ghost" onClick={() => setStatusFilter(null)}>
                All ({properties.length})
              </Button>
              <Button className="text-2xl" variant="ghost" onClick={() => setStatusFilter('available')}>
                Available ({properties.filter(property => property.status === 'available').length})
              </Button>
              <Button className="text-2xl" variant="ghost" onClick={() => setStatusFilter('rented')}>
                Rented ({properties.filter(property => property.status === 'rented').length})
              </Button>
              <Button className="text-2xl" variant="ghost" onClick={() => setStatusFilter('Booked')}>
                Booked ({properties.filter(property => property.status === 'booked').length})
              </Button>
            </div>
          </Tabs>
        </div>
