...
export default function TripIdPageClient({
  addListingToFavorites,
}: {
  listings: Listing[];
  addListingToFavorites: Function;
  tripId: string;
  trip: Trip;
}) {
    ...
    <>
      <>
        {!noMoreListings ? (
          <>
            <MatchBar
              setShowRankView={setShowRankView}
              currListing={listings[currIndex]}
          ...
          <>
            ...
            <button
              className="bg-primaryBrand py-1 px-2 m-2  border-black text-center text-xl"
              onClick={() => setCurrIndex(0)}
            >
              Start again?
            </button>
            <Link href={`platform/trips/${trip.id}/favorites`}>
              <button className="bg-primaryBrand py-1 px-2 m-2  border-black text-center text-xl">
                Rank your favorites
              </button>
            </Link>
          </>
