...
import { ChevronDownIcon, ChevronUpIcon, MoreVerticalIcon, ClipboardList, Calendar, Star, Home, Loader2, CreditCard, Database, Check, Square } from "lucide-react";
...
const OnboardingChecklistCard = ({ hostUserData, isAdminDev }: OnboardingChecklistCardProps): JSX.Element => {
  ...
  const optionalItems = [
    {
      id: 3,
      text: "Connect other listings with Hospitable",
      completed: true,
    },
  ];

  const renderChecklist = (items: typeof requiredItems, title: string) => (
        ...
        <div className="flex flex-col items-start gap-[18px] relative self-stretch w-full flex-[0_0_auto] mt-[18px]">
          <div className="flex flex-col items-start gap-2.5 relative self-stretch w-full flex-[0_0_auto]">
            ...
            {items.map((item) => {
              ...
              const shouldBeClickable = !item.completed && isStripeItem;
            ...
            {/* Optional section - Not ready yet */}
  ...
  if (isAdminDev) {
    return (
      <div className="flex flex-col gap-4">
        {renderChecklist(requiredItems, "Onboarding Checklist")}
        {renderChecklist(testRequiredItems, "Onboarding Checklist (Test)")}
      </div>
    );
  }

  // For regular users, just show the real version
  return renderChecklist(requiredItems, "Onboarding Checklist");
};
