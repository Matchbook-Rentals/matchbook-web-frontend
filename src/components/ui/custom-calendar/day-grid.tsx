...
const DayGrid: React.FC<DayGridProps> = ({ currentDate, bookings=[], unavailablePeriods=[] }) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (booking) {
      // If there's a booking, show PopoverDay with booking details
      days.push(<PopoverDay key={day} day={day} booking={booking} />);
    } else if (unavailability) {
      // If there's unavailability, show PopoverDay with unavailability details
      days.push(<PopoverDay key={day} day={day} unavailability={unavailability} />);
    } else {
