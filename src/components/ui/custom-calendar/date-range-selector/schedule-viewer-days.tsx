...
interface ScheduleViewerDaysProps {
  bookings?: Booking[];
  unavailablePeriods?: UnavailablePeriod[];
  onDeleteUnavailability?: (unavailabilityId: string) => void;
  onEditUnavailability?: (unavailability: UnavailablePeriod) => void;
}
                ...
                <DialogContent className="sm:max-w-[550px] flex flex-col !top-[15vh] sm:!top-[50%] !translate-y-0 sm:!translate-y-[-50%]" showCloseButton={false}>
                  ...
                  <div className="flex-1 py-4">
                    <p className="text-gray-600">
                      This will permanently remove the unavailable period from {unavailablePeriod.startDate.toLocaleDateString()} to {unavailablePeriod.endDate.toLocaleDateString()}. This action cannot be undone.
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-gray-200">
                    ...
                    <BrandButton
                      variant="destructive"
                      onClick={() => {
                        onDeleteUnavailability?.(unavailablePeriod.id);
...
export function CalendarMonth({ year, month, onPrevMonth, onNextMonth, isPrevDisabled, bookings = [], unavailablePeriods = [], onMonthChange, onYearChange, className, headerClassName, gridClassName, legendClassName, dayClassName, dayContainerClassName, daySpanClassName, useSelectPortal, onDeleteUnavailability, onEditUnavailability }: CalendarMonthProps) {
  ...
  const getUnavailablePeriod = (day: number) => {
    const currentDate = new Date(year, month, day);
    ...
    <div className={cn("w-full flex-1", className)}>
      ...
      <div className={cn("grid grid-cols-7 grid-rows-6 gap-y-1 h-[320px]", gridClassName)}>
        ...
        {Array.from({ length: daysInMonth }).map((_, index) => {
          ...
          const unavailablePeriod = getUnavailablePeriod(day);
...
export function ScheduleViewerDays({
  bookings = [],
  unavailablePeriods = [],
  onDeleteUnavailability,
  onEditUnavailability
}: ScheduleViewerDaysProps) {
