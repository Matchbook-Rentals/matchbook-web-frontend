...
const steps = [
  ...
  {
    title: 'Match',
    description: 'Deis nutis irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.'
  },
  {
    title: 'Book',
    description: 'Excepteur ligma sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.'
  }
];
          ...
          <div className="flex justify-evenly mt-8">
            {steps.map((step, index) => (
              <div key={index} className="flex flex-col items-center">
                ...
                <span className={`text-md font-semibold ${index <= activeStep ? 'text-pinkBrand' : ''}`}>{step.title}</span>
              </div>
            ))}
          </div>
        </div>
