...
function PaymentMethodForm({ matchId, amount, onSuccess, onCancel, clientSecret }: PaymentMethodFormProps) {
  ...
  const savePaymentMethodToMatch = async (paymentMethodId: string, matchId: string, amount: number) => {
    ...
    // Use verification endpoint if this is for verification
    const endpoint = matchId === 'verification'
      ? '/api/verification/charge-payment-method'
      : `/api/matches/${matchId}/payment-method`;
...
export function VerificationPaymentSelectorV2({ matchId, amount, onSuccess, onCancel, onPaymentMethodTypeChange }: PaymentMethodSelectorProps) {
